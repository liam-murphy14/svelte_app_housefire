...
<div class="w-full rounded-xl border border-hf-base-dark/20 bg-hf-base-light">
  {#if enablePagination}
    <div class="border-b border-hf-base-dark/20 p-4">
      <label class="flex max-w-sm flex-col gap-1 hf-body-2 text-hf-base-dark">
        Search properties
        <input
          type="search"
  ...
  {/if}
  <div class={enablePagination ? 'max-h-[36rem] overflow-auto' : 'overflow-x-auto'}>
    ...
    {#if enablePagination}
      <div
        class="sticky bottom-0 flex flex-wrap items-end justify-between gap-4 border-t border-hf-base-dark/20 bg-hf-base-light p-4"
      >
